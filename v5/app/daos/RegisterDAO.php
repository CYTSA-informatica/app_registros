...
final class RegisterDAO
{
    ...
    {
        $phpTimezoneOffset = date('P');
        try {
            $this->pdo->exec("SET time_zone = '{$phpTimezoneOffset}'");
        } catch (Throwable $e) {
            // Silently fail - timestamps will use MySQL's default timezone
        }
    }
    ...
    public function create(RegisterModel $register): void
    {
        ...
        try {
            $hasCustomFecha = !empty($register->fecha);
            
            if ($hasCustomFecha) {
                $stmt = $this->pdo->prepare(
                    'INSERT INTO registers (duracion, descripcion, estado, notas, id_empleado, fecha_inicio) VALUES (:duracion, :descripcion, :estado, :notas, :id_empleado, :fecha_inicio)'
                );
                $params = [
    ...
    public function update(RegisterModel $register): void
    {
        $this->pdo->beginTransaction();
        try {
            // Si se proporciona una fecha personalizada, actualizar fecha_inicio
            if (!empty($register->fecha)) {
                $stmt = $this->pdo->prepare(
                    'UPDATE registers SET duracion = :duracion, descripcion = :descripcion, estado = :estado, notas = :notas, fecha_inicio = :fecha_inicio WHERE id = :id'
                );
                $params = [
                    'id' => (int) $register->id,
                    'duracion' => $register->duracion,
    ...
    public function findAllForUser(int $userId, bool $isAdmin): array
    {
        if ($isAdmin) {
            $stmt = $this->pdo->query(
                     'SELECT r.*, COALESCE(rca.clientes_nombres, c_legacy.nombre, "") AS clientes_nombres, COALESCE(rpa.proveedores_nombres, "") AS proveedores_nombres, u.username AS empleado_nombre, u.dpto AS empleado_dpto
                 FROM registers r
                 LEFT JOIN (
