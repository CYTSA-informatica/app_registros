...
function try_restore_user_from_remember_cookie(): ?array
{
    ...
    $stmt = app_pdo()->prepare(
        'SELECT rt.id, rt.user_id, rt.token_hash, rt.expires_at, u.id, u.username, u.email, u.isAdmin
...
function login_user(string $email, string $password): bool
{
    ...
    $stmt = app_pdo()->prepare('SELECT id, username, email, contra_hash, isAdmin FROM users WHERE username = :identifier LIMIT 1');
    ...
    $stmt = app_pdo()->prepare('SELECT id, username, email, contra_hash, isAdmin FROM users WHERE email = :email ORDER BY username ASC');
